...
使用 <b>“http://[转码nodejs服务器]:7677/api/stream?channel=[频道名称]” </b> 访问转码服务器后，nodejs收到请求，会把对应的真实的 udpxy的 http地址进行请求转发，获取对应 mpeg 格式下的 ts 数据块。然后利用ffmpeg工具，进行媒体流进行转码，生成针对cctv1本地的m3u8文件和 对应本地*.ts数据流媒体文件。然后在利用请求重定向 把上面的请求 重定向到 `http://${req.hostname}:${PORT}/${dist}/${ch}.m3u8` 静态资源上。这样iphone 和 ipad 就可以播放了。同时请注意多个终端请求转码服务器同一个频道，服务器只会增开一个子进程去针对同一个频道进行转码，比如cctv1，就只会增加1个子进程去转cctv1的节目，每个终端访问，都会和服务器保持一个叫做 "cctv1"的心跳。一旦服务器发现没有任何终端访问cctv1，转码服务器就会停止对该频道的转码，如果有100个终端同时访问100个不同的频道，那就会开启100个子进程去针对不同频道的转码。所以要求运行转码服务器需要有一定CPU处理能力，不推荐安装部署在较弱的 路由或者NAS上。
